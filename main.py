...
class Root(Tk):
    def __init__(self, boardSize = 8) -> None:
        ...
        self.board = [[None for row in range(boardSize)] for col in range(boardSize)]
...
class SquareButton:
    def __init__(self, canvas: tkinter.Canvas, row: int, column: int, size: int, color: str) -> None:
        ...
        x2 = x1 + self.size
        y2 = y1 + self.size
        canvas.create_rectangle(x1, y1, x2, y2, fill=self.color, tags=f"tile{self.column}{self.row}", outline="")
        canvas.tag_bind(f"tile{self.column}{self.row}", "<Button-1>", self.__str__)

    def __str__(self, event=None):
        print(f"{chess.FILE_NAMES[self.row]}{chess.RANK_NAMES[7 - self.column]}")
